...
try:
    ...
    def load_mat_file(path, varname):
        ...
        h5py_flg = MAT_H5PY_FLG[cur_key]
        if h5py_flg:
            try:
                return sio.loadmat(path)[varname]
            except:
                h5py_flg = not h5py_flg
                MAT_H5PY_FLG[cur_key] = h5py_flg
        if not h5py_flg:
            try:
                f = h5py.File(path, 'r')
                data = np.array(f[varname])
                f.close()
                data = data.transpose(range(len(data.shape))[::-1])
                return data
            except:
                h5py_flg = not h5py_flg
                MAT_H5PY_FLG[cur_key] = h5py_flg
...
def get_html(url, cache=False):
    fname = None
    if cache:
        WEB_DIR = '.web_cache'
        mkdir_if_missing(WEB_DIR)
        md5_hash = hashlib.md5(url).hexdigest()
        fname = os.path.join(WEB_DIR, md5_hash)
        if os.path.exists(fname):
            return textread(fname)[0]
